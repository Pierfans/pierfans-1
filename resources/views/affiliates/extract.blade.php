...
<head>
    ...
    <title>Extrato de Afiliados - {{ config('app.name', 'Laravel') }}</title>
...
<body class="bg-[#FDFDFC] text-[#1b1b18] min-h-screen">
    ...
    <div class="pt-0 md:pt-16 pb-16 md:pb-0">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <!-- Header -->
            <div class="mb-8">
                <h1 class="text-3xl font-bold text-[#1b1b18] mb-2">Extrato de Afiliados</h1>
                <p class="text-[#706f6c]">Visualize todas as suas transações de afiliado</p>
            </div>
            ...
            <div class="extract-card mb-6">
                ...
                <form method="GET" action="{{ route('affiliates.extract') }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="commission_type" class="input-field">
                            <option value="all" {{ ($commissionType ?? 'all') === 'all' ? 'selected' : '' }}>Todas</option>
                            <option value="subscription" {{ ($commissionType ?? 'all') === 'subscription' ? 'selected' : '' }}>Por Assinante</option>
                            <option value="creator_sale" {{ ($commissionType ?? 'all') === 'creator_sale' ? 'selected' : '' }}>Por Criador de Conteúdo</option>
                        </select>
                    </div>
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex-1">
                                        <div class="flex items-center gap-2 mb-1 flex-wrap">
                                            ...
                                            @if($transaction['type'] === 'withdrawal')
                                                ...
                                                @endif
                                            @else
                                                @if($transaction['status'] === 'liberado')
                                                    <span class="status-badge status-complete">{{ $transaction['status_label'] }}</span>
                                                @else
                                                    <span class="status-badge status-pending">{{ $transaction['status_label'] }}</span>
                                                @endif
                                                @if(isset($transaction['commission_type']))
                                                    <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium 
                                                        {{ $transaction['commission_type'] === 'Assinatura' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800' }}">
                                                        {{ $transaction['commission_type'] === 'Assinatura' ? '📥 Por Assinante' : '💰 Por Criador' }}
                                                    </span>
                                                @endif
                                            @endif
                                        </div>
                                        <p class="text-sm text-[#706f6c]">
                                            {{ $transaction['date_string'] ?? (is_object($transaction['date']) ? $transaction['date']->format('d M, Y, h:i A') : $transaction['date']) }}
                                        </p>
                                        @if($transaction['type'] === 'commission' && isset($transaction['plan_name']))
                                            <p class="text-xs text-[#706f6c] mt-1">
                                                {{ $transaction['plan_name'] }} - {{ $transaction['creator_name'] }}
                                            </p>
                                        @endif
                                    </div>
                                    <div class="flex items-center gap-4">
                                        ...
                                        @if($transaction['type'] === 'commission')
                                            <a 
                                                href="{{ route('affiliates.commission.details', str_replace('comm_', '', $transaction['id'])) }}"
    ...
    <script>
        // Dados das transações para JavaScript (apenas saques)
        const transactions = @json($transactionsForJs ?? []);
        
        // Dados completos de todas as transações (incluindo comissões)
        const allTransactionsData = @json($allTransactionsForJs ?? []);
        ...
        function openDetailsModal(transactionId, transactionType) {
            ...
            if (transaction.type === 'withdrawal') {
                // Detalhes de saque
                const withdrawal = transaction.data;
                const statusLabels = {
                    'pending': 'Pendente',
                ...
                html = `
                    <div class="space-y-4">
                        ...
                        <div class="flex justify-between">
                            ...
                            <span class="font-semibold text-[#1b1b18]">${formatDateTime(transaction.data.created_at)}</span>
                        </div>
                ...
                const commissionTypeLabel = transaction.commission_type === 'Assinatura' ? 'Por Assinante' : 'Por Criador de Conteúdo';
                
                html = `
                    <div class="space-y-4">
                        <div class="flex justify-between">
                            <span class="text-[#706f6c]">Tipo de Comissão</span>
                            <span class="font-semibold text-[#1b1b18]">${commissionTypeLabel}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-[#706f6c]">Valor da Comissão</span>
                            <span class="font-semibold text-[#1b1b18]">R$ ${parseFloat(transaction.amount).toFixed(2).replace('.', ',')}</span>
                        </div>
                        ${transaction.plan_name ? `
                        <div class="flex justify-between">
                            <span class="text-[#706f6c]">Plano</span>
                            <span class="font-semibold text-[#1b1b18]">${transaction.plan_name}</span>
                        </div>
                        ` : ''}
                        ${transaction.creator_name ? `
                        <div class="flex justify-between">
                            <span class="text-[#706f6c]">Criador</span>
                            <span class="font-semibold text-[#1b1b18]">${transaction.creator_name}</span>
                        </div>
                        ` : ''}
                        <div class="flex justify-between">
                            <span class="text-[#706f6c]">Data</span>
                            <span class="font-semibold text-[#1b1b18]">${transaction.date_string || formatDateTime(transaction.date)}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-[#706f6c]">Status</span>
                            <span class="font-semibold text-[#1b1b18]">${transaction.status_label}</span>
                        </div>
                    </div>
                `;
            }
