...
<body class="bg-[#FDFDFC] min-h-screen flex flex-col">
    ...
    <div class="flex flex-col lg:flex-row" style="min-height: 480px;">
        ...
        <!-- Coluna Direita: banner lancamento das collab "Mansao da Juju" (pedido do Bento 19/08:
             foto nova + "Sexta-feira / O grande lancamento de todas as collab da Mansao da Juju").
             Texto em HTML pra nunca mais ficar refem de arte chapada no JPG. -->
        ...
        {{-- div e nao <a> (28/08): os @ da frase sao links proprios, e link dentro de link nao existe em HTML.
             Foto e botao levam pro perfil do campo; o degrade nao pode roubar o clique da foto --}}
        <div class="relative block w-full lg:w-1/2 overflow-hidden bg-[#01313B] aspect-[3/2] lg:aspect-auto">
            <a href="{{ route('profile.show', $banner['username']) }}" class="absolute inset-y-0 right-0 h-full w-3/5 lg:w-[65%]">
                <img src="{{ $banner['image'] }}" alt="{{ $banner['text'] }}" class="h-full w-full object-cover">
            </a>
            ...
            <div class="relative h-full flex flex-col justify-center pl-6 pr-3 lg:pl-8 xl:pl-12 lg:pr-4 w-[45%] lg:w-[34%]">
                ...
                <a href="{{ route('profile.show', $banner['username']) }}" class="mt-4 lg:mt-6 self-start inline-flex items-center gap-1.5 bg-[#14d1bc] hover:bg-[#0e9486] text-[#01323a] hover:text-white font-semibold text-xs lg:text-sm px-4 py-2 lg:py-2.5 rounded-lg transition-colors">
                    ver o perfil →
                </a>
            </div>
        </div>
