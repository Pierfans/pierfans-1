    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="bg-white rounded-lg shadow-sm p-4 mb-6">
            <form method="GET" action="{{ route('admin.vendas.index') }}" class="flex flex-wrap items-end gap-3">
                ...
                {{-- grupo é controlado pelas abas (links); hidden pra o botão Filtrar preservar a aba atual --}}
        ...
        {{-- Abas de agrupamento: cada uma preserva os filtros atuais (from/to/tipo/criador) --}}
        <div class="flex flex-wrap gap-x-6 gap-y-1 mb-4 border-b border-gray-200">
            @foreach(['criador' => 'Criador', 'cliente' => 'Cliente', 'afiliado' => 'Afiliado', 'dia' => 'Dia', 'mes' => 'Mês'] as $g => $label)
                <a href="{{ route('admin.vendas.index', array_merge(request()->query(), ['grupo' => $g])) }}"
                   class="px-1 py-2 -mb-px text-sm font-medium border-b-2 {{ $grupo === $g ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-800' }}">
                    {{ $label }}
                </a>
            @endforeach
        </div>
